...
| package | spec | status |
|---|---|---|
| `zndx.engine.v1` | [`specification/protocol/engine_grpc.md`](specification/protocol/engine_grpc.md) | v1 — `Complete` + `Status` + `Remediate` + **`Yield`** + `ServerQuery` + `RecordLineage` + `WatchWorkload` + **`Announce`** (TTL’d S2S join); **operating profiles** (`instruct` = same model, effort low — [`capabilities.md`](specification/protocol/capabilities.md)); warehouse `tx_id` is RFC 9562 UUIDv7 ([`tx_id.md`](specification/protocol/tx_id.md)); data products on shared RustFS ([`data_products.md`](specification/protocol/data_products.md)) with **SHACL Core Aspects** (`NodeShape` / `ProductSpec` / `AspectBinding`) and Agenda **attachments**; primary UI discovery in [`surfaces.md`](specification/protocol/surfaces.md) |
...
| coordination Activities | [`specification/protocol/coordination_activities.md`](specification/protocol/coordination_activities.md) | inter-project **intent with a lifetime** as Signals Airflow runs: `zndx.engine.v1.Activity` + `Scheduler/{Declare,Renew,Release,List,Watch}Activity` + supervision `ActivityEvent`; two hops, never one — local processes → local engine → Signals engine → Airflow |
| `zndx.agent.v1` | [`specification/protocol/agent_grpc.md`](specification/protocol/agent_grpc.md) | v1 **draft** (2026-09-09) — **agents** as the third capability class (Holland / JADE: the bounded assemblage of runtime + tools + workspace + the model capability it thinks with): `ListAgents`, `Run` (stream), `Cancel`, `GetRun`; Harbor `--agent`/`--model` alignment; runs are Activities; trajectories (ATIF) are data products |
